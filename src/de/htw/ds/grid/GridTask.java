...
import de.sb.javase.xml.PathContentAdapter;
...
public final class GridTask implements Runnable, Serializable {
	...
	@XmlJavaTypeAdapter(PathContentAdapter.class)
	@XmlElement(name="jarContent")
	 ...
	 */
	public GridTask(final URI responseURI, final String agentClassName, final Path jarPath, final Path dataPath, final Map<String,String> properties, final int tryCount) throws NoSuchFileException, AccessDeniedException {
		super();
	...
	private boolean respond(final GridTaskResponse response) {
		...
		for (int tryIndex = 0; tryIndex < this.tryCount; ++tryIndex) {
			try {
				final URL wsdlLocator = new URL(this.responseURI.toASCIIString() + "?wsdl");
				...
				proxy.processTaskResponse(response);
				return true;
			...
			try {
				Thread.sleep(TimeUnit.SECONDS.toMillis(15) << tryIndex);
			} catch (final InterruptedException nestedException) {
