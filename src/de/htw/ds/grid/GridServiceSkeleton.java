...
public interface GridServiceSkeleton {
	 ...
	 * @throws WebServiceException if there's a JAX-WS related problem
	 */
	boolean hasActiveTask(long taskIdentity);
	 ...
	 */
	long createTask(String agentClassName, Path jarPath, Path dataPath, Map<String,String> properties, int tryCount, int timeToLive) throws NoSuchFileException, AccessDeniedException;
	 ...
	 * @throws WebServiceException if there's a JAX-WS related problem
	 */
	void processTask(GridTaskSkeleton task);
	 ...
	 * @throws WebServiceException if there's a JAX-WS related problem
	 */
	void processTaskResponse(GridTaskResponse taskResponse);
}